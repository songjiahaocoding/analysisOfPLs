...
public class App1 {
    public void getWC(String file_path) throws IOException {
        HashMap<String, Integer> map = new HashMap<>();
        HashMap<String, Integer> set = new HashMap<>();
        buildFreqs(map, new String(Files.readAllBytes(Paths.get(file_path))).toLowerCase(Locale.ROOT));
        ...
        list.sort(Comparator.comparingInt(AbstractMap.SimpleEntry::getValue));
        for (int i=0;i<25;i++) System.out.println(list.get(list.size()-i-1).getKey()+" - "+list.get(list.size()-i-1).getValue());
    }

    public void buildFreqs(HashMap<String, Integer> map, String data){
